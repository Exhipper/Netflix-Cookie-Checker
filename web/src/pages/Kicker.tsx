...
import { useCheckRun, type PukeFile } from "@/hooks/useCheckRun";
...
export default function Kicker() {
  ...
  // If a run is active with pukes from the context, show those
  const displayPukes = isRunning && runPukes.length > 0 ? runPukes : pukes;
  ...
  const removePuke = (index: number) => {
    setPukes((prev) => prev.filter((_, i) => i !== index));
  };

  const handleStartCheck = async () => {
    if (pukes.length === 0) {
      toast.error("Please add at least one puke file");
      return;
    }
    try {
      const result = await start(pukes, proxyText, config || {}, threads);
      toast.success(`Check started: ${result.total} pukes, ${result.threads} threads`);
    } catch (err: any) {
  ...
  const total = progress?.total || displayPukes.length;
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto animate-fade-in">
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight">Puke Kicker</h1>
        <p className="text-muted-foreground mt-1">
          Upload NFX puke files and check them against the NFX API
        </p>
        {isRunning && (
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-lg">
                ...
                Puke Files
                {displayPukes.length > 0 && (
                  <Badge variant="secondary" className="ml-1">{displayPukes.length}</Badge>
                )}
              </CardTitle>
            </CardHeader>
            <CardContent>
              ...
              >
                ...
                <p className="text-sm font-medium">Drop puke files here or click to browse</p>
                  ...
                  {displayPukes.map((puke, i) => (
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="text-sm truncate">{puke.name}</span>
                        <span className="text-xs text-muted-foreground shrink-0">
                          ({(puke.size / 1024).toFixed(1)} KB)
                        </span>
                      </div>
                      {!isRunning && (
                        <Button
            ...
            <CardContent className="p-5">
              {!isRunning ? (
                <Button
                  ...
                  disabled={pukes.length === 0}
                >
                  <Play className="h-5 w-5 mr-2" />
                  Start Check ({pukes.length} files)
                </Button>
              ) : (
                <Button
