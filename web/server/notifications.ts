...
export function formatCookieFile(
  ...
  lines.push("", divider, "", "NFX PUKE :👇", "", cookieContent.trim(), "");
  return lines.join("\n");
}
...
function buildDiscordFullMessage(
  ...
  const lines = [
    "# [NFX Puke Kicker](https://github.com/Exhipper/NFX-Puke-Kicker)",
    "", "Puke details",
  ];
...
function buildDiscordCookieMessage(cookieContent: string): string {
  return [
    "# [NFX Puke Kicker](https://github.com/Exhipper/NFX-Puke-Kicker)",
    "", "Puke details", "```txt", cookieContent.trim(), "```",
...
function buildDiscordNfTokenMessage(
  ...
  const lines = ["# [NFX Puke Kicker NFToken](https://github.com/Exhipper/NFX-Puke-Kicker)", ""];
  const links = buildNfTokenLinks(nfTokenData.token, nfTokenMode);
  if (links.length) {
...
function buildTelegramFullMessage(
  ...
  const lines = [
    '<b><a href="https://github.com/Exhipper/NFX-Puke-Kicker">NFX Puke Kicker</a></b>',
    "", "<b>Puke details</b>",
  ];
  for (const line of buildNotificationDetails(config, info, isSubscribed)) {
    const [label, ...rest] = line.split(": ");
...
function buildTelegramNfTokenMessage(
  ...
  const lines = ['<b><a href="https://github.com/Exhipper/NFX-Puke-Kicker">NFX Puke Kicker NFToken</a></b>', ""];
  const links = buildNfTokenLinks(nfTokenData.token, nfTokenMode);
  if (links.length) {
