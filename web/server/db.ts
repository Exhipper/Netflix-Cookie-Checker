...
/** Update only the NFToken data and formatted output for a result (used for background token generation). */
export async function updateNfTokenData(
  resultId: number,
  nfTokenData: { token: string; expires_at_utc: string },
  formattedOutput?: string
): Promise<void> {
  const pool = getPool();
  await pool.query(
    `UPDATE results SET nftoken_data = $1, formatted_output = COALESCE($2, formatted_output) WHERE id = $3`,
    [JSON.stringify(nfTokenData), formattedOutput || null, resultId]
  );
}
